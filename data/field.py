...
from .randaug import RandAugment, MinMaxResize, MaxWHResize
...
class ImageAllFieldWithMask(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None,max_detections=100,
                 load_in_tmp=True):
        self.max_detections = max_detections
        self.detections_path = detections_path
        ...
        super(ImageAllFieldWithMask, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            f = h5py.File(self.detections_path, 'r')
            precomp_data = f['%d_features' % image_id][()]
            grid_data = f['%d_grids' % image_id][()]
            boxes = f['%d_boxes' % image_id][()]
            size = f['%d_size' % image_id][()]
            mask = f['%d_mask' % image_id][()]
        except KeyError:
            warnings.warn('Could not find detections for %d' % image_id)
            precomp_data = np.random.rand(10, 2048)
            boxes = np.random.rand(10, 4)
            size = np.random.rand(10, 2)
            raise
        size = size[:, ::-1]
        ...
        return precomp_data.astype(np.float32), relative_boxes.astype(np.float32), grid_data.astype(np.float32), mask.astype(np.float32)

class ImageSwinRegionGridWithMask(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=None, load_in_tmp=True):
        self.region_path = os.path.join(detections_path, "feature", "region")
        self.grid_path = os.path.join(detections_path, "feature", "grid")

        super(ImageSwinRegionGridWithMask, self).__init__(preprocessing, postprocessing)

    def preprocess(self, x, avoid_precomp=False):
        image_id = int(x.split('_')[-1].split('.')[0])
        with np.load(os.path.join(self.region_path, str(image_id) + '.npz'), allow_pickle=True) as data_region:
            region = data_region['region']
            region = np.array(region).astype('float32')
            region_mask = data_region['mask']
            region_mask = np.array(region_mask).astype('bool')
        with np.load(os.path.join(self.grid_path, str(image_id) + '.npz'), allow_pickle=True) as data_grid:
            grid = data_grid['grid']
            grid = np.array(grid).astype('float32')
            grid_mask = data_grid['mask']
            grid_mask = np.array(grid_mask).astype('bool')
        return region, region_mask, grid, grid_mask

class ImageSwinRegionWithMask(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=None, load_in_tmp=True):
        self.region_path = os.path.join(detections_path, "feature", "region")

        super(ImageSwinRegionWithMask, self).__init__(preprocessing, postprocessing)
...
class ImageSwinGridWithMask(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=None, load_in_tmp=True):
        self.region_path = os.path.join(detections_path, "feature", "grid")

        super(ImageSwinGridWithMask, self).__init__(preprocessing, postprocessing)

    def preprocess(self, x, avoid_precomp=False):
        image_id = int(x.split('_')[-1].split('.')[0])
        with np.load(os.path.join(self.region_path, str(image_id) + '.npz'), allow_pickle=True) as data_grid:
            grid = data_grid['grid']
            grid = np.array(grid).astype('float32')
            grid_mask = data_grid['mask']
            grid_mask = np.array(grid_mask).astype('bool')
